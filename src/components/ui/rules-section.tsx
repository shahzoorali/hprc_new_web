...
const getCategoryTheme = (category: string) => {
  ...
  if (
    lowerCategory.includes("etiquette") ||
    lowerCategory.includes("conduct") ||
    lowerCategory.includes("ground")
  ) {
    return {
      bg: "bg-violet-50",
    ...
    <section className="container space-y-8 sm:space-y-12">
      {(eyebrow || title || description) && (
        <div className="max-w-3xl space-y-3 sm:space-y-4">
          ...
          {title && (
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-brand-900">{title}</h2>
          )}
          {description && (
        ...
        {categories.map((ruleCategory, categoryIndex) => {
          ...
          return (
            ...
            >
              {/* Category Header */}
              <div
                className={`relative overflow-hidden bg-gradient-to-r ${theme.bg} px-6 py-5 sm:py-6`}
              >
                ...
                <div className="relative flex items-center gap-4">
                  <div
                    className={`flex h-12 w-12 items-center justify-center rounded-xl ${theme.iconBg} ${theme.iconText} shadow-lg transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3`}
                  >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      {ruleCategory.items.length}{" "}
                      {ruleCategory.items.length === 1 ? "rule" : "rules"}
                    </p>
                  </div>
                  <div
                    className={`hidden sm:flex h-10 w-10 items-center justify-center rounded-full ${theme.badge} text-white font-bold`}
                  >
                  ...
                  {ruleCategory.items.map((item, itemIndex) => (
                    ...
                    >
                      <div className="flex-shrink-0 mt-0.5">
                        <div
                          className={`flex h-8 w-8 items-center justify-center rounded-lg ${theme.iconBg} ${theme.iconText} font-bold transition-all duration-300 group-hover/item:scale-110 group-hover/item:${theme.badge} group-hover/item:text-white`}
                        >
                          {itemIndex + 1}
                        </div>
                      </div>
                      ...
                      <div
                        className={`absolute right-3 top-3 h-1.5 w-1.5 rounded-full ${theme.badge} opacity-0 transition-opacity duration-300 group-hover/item:opacity-100`}
                      />
                    </div>
                  ))}
                </div>
