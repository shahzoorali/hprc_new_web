...
export function SectionHeading({
  ...
  return (
    <div className={`max-w-3xl ${alignment} space-y-3 ${className}`}>
      {eyebrow ? (
        <p className={`text-xs sm:text-sm font-semibold uppercase tracking-[0.2em] sm:tracking-[0.3em] ${eyebrowColor}`}>{eyebrow}</p>
      ) : null}
      <h2 className={`text-2xl sm:text-3xl md:text-4xl font-semibold ${titleColor}`}>{title}</h2>
      {description ? <p className={`text-sm sm:text-base ${descriptionColor}`}>{description}</p> : null}
    </div>
  );
}
