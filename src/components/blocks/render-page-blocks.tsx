...
// Renders an event page's `body` blocks. Each case hands off to the component
// that already draws that section on the hand-built pages, so a CMS-built event
// page produces the same markup as a coded one.

type PageBlock = NonNullable<Event["body"]>[number];
...
async function ResultsBoardFromSet({ setId }: { setId: string }) {
  ...
  const p = await payload.getPayload({ config });
  const set = await p.findByID({ collection: "result-sets", id: setId, depth: 0 });
  if (!set) return null;
  const data = await getResultSet(set.slug);
  if (!data) return null;
  return <ResultsBoard classes={data.classes} showPhotos={data.showPhotos} />;
}

export async function RenderPageBlocks({ blocks }: { blocks?: PageBlock[] | null }) {
