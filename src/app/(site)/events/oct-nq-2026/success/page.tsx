...
function SuccessContent() {
  ...
  useEffect(() => {
    if (!isSuccess) return;
    try {
      const regs = JSON.parse(localStorage.getItem("oct-nq-2026-registrations") ?? "[]");
      const last = regs[regs.length - 1];
      if (last) {
        localStorage.setItem("oct-nq-2026-confirmed", JSON.stringify({
          name: last.name,
  ...
  return (
    <div className="space-y-16 pb-24">
      <div className="container pt-16">
        <PageHero
          eyebrow="National Qualifier (NQ) October 2026"
          title={isSuccess ? "Entry Confirmed!" : "Payment Status"}
          description={
            isSuccess
              ? "Thank you! Your entry for the National Qualifier (NQ) October 2026 has been received and verified."
...
export default function OctNqSuccessPage() {
  return (
    <Suspense fallback={
