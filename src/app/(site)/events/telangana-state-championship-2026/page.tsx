...
type FormData = {
  ...
  stablingType: "NONE" | "PERMANENT" | "TEMPORARY" | "FULL_CAMP";
...
// cover the Championship dates only (16–19 Oct); FULL_CAMP is the shared
// NQ + Championship package (14–19 Oct) — identical total/dates to the October
// NQ page's own Full Camp package, so a rider who already booked it there is
// covered here too (see the NQ recall below). All three share the same
// InventoryManagerOctCamp ledger on the backend.
const STABLING_PACKAGES = {
  PERMANENT: { total: 7500, from: "2026-10-16", to: "2026-10-19" },
  TEMPORARY: { total: 3000, from: "2026-10-16", to: "2026-10-19" },
  FULL_CAMP: { total: 10000, from: "2026-10-14", to: "2026-10-19" },
} as const;
...
const STABLING_API_URL = "https://hprc.in/payment/api/stabling-available-oct-camp.php";
...
function RegistrationForm() {
  ...
  // National Qualifier (Oct 2026) recall — if the rider confirmed an NQ entry on
  // this device, offer to import their details, and auto-cover stabling if they
  // booked Full Camp (same package, same ledger — see STABLING_PACKAGES above).
  ...
  React.useEffect(() => {
    ...
    try {
      const existing = JSON.parse(localStorage.getItem("tsec2026-registrations") ?? "[]");
      if (existing && existing.length > 0) setDraftExists(true);
      const nq = JSON.parse(localStorage.getItem("oct-nq-2026-confirmed") ?? "null");
  ...
  // Standard entries close 18:00, Thu 15 Oct 2026. Post-entries accepted online
  // until noon, Fri 16 Oct 2026 (per prospectus §6c/d) — spot entries thereafter
  // are handled at the venue, not through this form.
  type EntryStatus = "STANDARD" | "POST_ENTRY" | "CLOSED";
  const entryStatus: EntryStatus = useMemo(() => {
    const now = new Date();
    if (now > new Date("2026-10-16T12:00:00+05:30")) return "CLOSED";
    if (now > new Date("2026-10-15T18:00:00+05:30")) return "POST_ENTRY";
    return "STANDARD";
  }, []);
  ...
  const validate = () => {
    ...
    // Day 1 (16 Oct) & Day 3 (18 Oct): max 2 per horse.
    // Day 2 (17 Oct): max 3 per horse across Dressage + Show Jumping
    // (one additional Hacks entry is permitted and is not counted toward the cap).
    const dayCaps: Record<string, number> = { "16 Oct": 2, "17 Oct": 3, "18 Oct": 2 };
    const horseCounts: Record<string, Record<string, number>> = {};

    activeSelected.forEach(id => {
      const ev = eligibleEvents.find(eve => eve.id === id);
      if (!ev) return;
      if (ev.date === "17 Oct" && ev.discipline === "HACKS") return; // additional Hacks entry
      const horses = form.eventHorses[id] || [];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("tsec2026-registrations", JSON.stringify(existing));
    } catch {}
  ...
  return (
    <div className="relative">
      ...
      action="https://hprc.in/payment/tsec2026RequestHandler.php"
      method="POST"
      encType="multipart/form-data"
      className="space-y-10 relative"
      noValidate>
      <fieldset disabled={entryStatus === "CLOSED"} className={`space-y-10 ${entryStatus === "CLOSED" ? "opacity-40 select-none" : ""}`}>
      ...
      {nqProfile && !nqImported && (
        <div className="bg-emerald-50 border border-emerald-200 p-4 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-emerald-900">National Qualifier (Oct) entry found{nqProfile.name ? ` — ${nqProfile.name}` : ""}</p>
        ...
        {fullCampCovered && (
          <div className="bg-green-50 border border-green-200 p-5 rounded-2xl flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-green-700 mt-1">Your Full Camp (14–19 Oct) booked with the National Qualifier covers the State Championship — no additional stabling fee here.</p>
            </div>
          </div>
        )}
        {!fullCampCovered && (
        <div className="bg-blue-50/50 border border-blue-100 p-5 sm:p-6 rounded-2xl space-y-5">
          <p className="text-xs text-blue-700 font-medium">Stables are limited and allocated first-come, first-served. Choose a package below — fees are per stable.</p>
            ...
            <div className="sm:col-span-2">
              ...
              <div className="grid sm:grid-cols-4 gap-2">
                {([
                  ...
                  { type: "PERMANENT", title: "Permanent · 16–19 Oct", sub: "₹7,500 total" },
                  { type: "TEMPORARY", title: "Temporary · 16–19 Oct", sub: "₹3,000 total" },
                  { type: "FULL_CAMP", title: "NQ + Championship · 14–19 Oct", sub: "₹10,000 total" },
                ] as const).map((opt) => (
                  <button
                    key={opt.type}
                    type="button"
            ...
            {form.stablingType !== "NONE" && (
              <div className="sm:col-span-2 flex items-center gap-2 text-xs text-blue-700 font-medium bg-white border border-blue-100 px-4 py-2.5">
                ...
                {form.stablingType === "PERMANENT"
                  ? "Permanent Stable · 16th – 19th October 2026 · ₹7,500 total"
                  : form.stablingType === "TEMPORARY"
                    ? "Temporary Stable · 16th – 19th October 2026 · ₹3,000 total"
                    : "NQ + Championship Full Camp · 14th – 19th October 2026 · ₹10,000 total"}
              </div>
            )}
          </div>
      ...
      <div className="text-center pt-2">
        {entryStatus === "CLOSED" ? (
          <div className="bg-red-50 border border-red-200 p-6 max-w-lg mx-auto">
            ...
            <p className="text-xs text-red-600">The post-entry deadline (16 Oct, 12:00 noon) has passed. We are no longer accepting online entries — spot entries may still be available at the venue.</p>
          </div>
        ) : (
          <>
            <button
          ...
          <div className="relative overflow-hidden border-2 border-brand-200/50 shadow-[0_40px_80px_-20px_rgba(227,30,36,0.3)]">
            ...
            <div className="relative z-10 px-6 sm:px-12 md:px-20 pb-12 sm:pb-16 pt-20 sm:pt-24 text-center">
              ...
              <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold text-white font-display tracking-tight leading-tight">
                Telangana State
                <br />
                <span className="text-brand-300">Equestrian Championship</span>
              </h1>
        ...
        <div className="relative overflow-hidden border-2 border-brand-100 bg-white p-6 sm:p-10 md:p-14 shadow-2xl">
          ...
          <div className="relative">
            <div className="text-center mb-10">
              ...
              <h2 className="text-3xl sm:text-4xl font-extrabold text-brand-900 font-display">Register for the Championship</h2>
              <p className="mt-3 text-sm text-gray-500 max-w-lg mx-auto">
                Fill in the form below and proceed to payment to complete your registration.
              </p>

              {/* Entry Deadline Alert */}
              <div className="mt-8 max-w-sm mx-auto">
                <div className="bg-brand-50 border-l-4 border-brand-500 p-4 text-left">
                  <p className="text-xs font-bold text-brand-700 uppercase tracking-wider mb-1">Standard Entries Close</p>
                  <p className="text-sm font-bold text-brand-900">Thursday, 15 Oct · 6:00 PM</p>
                  <p className="text-xs text-brand-700 mt-1">Post-entries accepted until noon, Friday 16 Oct (surcharge applies).</p>
                </div>
              </div>
