...
function SuccessContent() {
  ...
  // On a confirmed NQ entry, persist a lightweight profile so the Equestrian
  // Challenge form can recall the rider's details and detect a Full Camp booking.
  useEffect(() => {
    if (!isSuccess) return;
    try {
      const regs = JSON.parse(localStorage.getItem("nq-2026-registrations") ?? "[]");
      const last = regs[regs.length - 1];
      if (last) {
        localStorage.setItem("nq-2026-confirmed", JSON.stringify({
          name: last.name,
          parentName: last.parentName,
          dob: last.dob,
          mobile: last.mobile,
          email: last.email,
          emergencyContact: last.emergencyContact,
          emergencyRelation: last.emergencyRelation,
          clubName: last.clubName,
          horses: Object.values(last.eventHorses ?? {}).flat().filter(Boolean),
          stablingType: last.stablingType,
          stablingFrom: last.stablingFrom,
          stablingTo: last.stablingTo,
  ...
  return (
    <div className="space-y-16 pb-24">
      <div className="container pt-16">
        <PageHero
          eyebrow="National Qualifier (NQ) 2026"
          title={isSuccess ? "Entry Confirmed!" : "Payment Status"}
          description={
            isSuccess
              ? "Thank you! Your entry for the National Qualifier (NQ) 2026 has been received and verified."
