...
export default async function NQ2026ResultsPage() {
  const resultSet = await getResultSet("nq-2026");
  ...
  return (
    <div className="space-y-16 pb-16">
      ...
      <section className="container">
        <div className="rounded-[2.5rem] border-2 border-brand-100/70 bg-gradient-to-br from-brand-50/80 via-white to-brand-50/60 p-8 shadow-xl md:p-12">
          ...
          <div className="mx-auto mt-6 max-w-3xl space-y-3 text-center text-gray-700">
            <p>
              Results are listed as scored, not ranked. Dressage classes show the average
              judges&apos; score — a higher percentage reflects a stronger round. Show Jumping
              classes show total penalties, where a lower figure reflects a cleaner round.
            </p>
          </div>
        </div>
      </section>
      ...
      {/* Results */}
      <section className="container">
        <ResultsBoard classes={classes} showPhotos={false} showRanking={false} />
      </section>
      ...
      <section className="container">
        <div className="space-y-4 rounded-[2rem] bg-brand-900 p-8 text-center md:p-12">
          <p className="mx-auto max-w-3xl text-lg leading-relaxed text-white/90">
            HPRC congratulates every rider who qualified and competed. The National Qualifier
            remains a key step on the pathway to national-level equestrian competition in India.
          </p>
