    ...
    if (hprcMatch) {
      hprcMatch.forEach((section, index) => {
        ...
        if (contentMatch) {
          ...
          const dateMatch = content.match(/(\d{1,2}\s+(?:January|February|March|April|May|June|July|August|September|October|November|December)[a-z]+),?s+(\d{1,2}(?:st|nd|rd|th)[a-z]+),?s+(\d{4})/g);
          
          let title = '';
          // Extract heading/title
          const headingMatch = content.match(/(?:October|November|December|January|February|March|April|May|June|July|August|September)\s+\d{1,2}\s+(?:st|nd|rd|th)/i);
