  ...
  .get("https://hprc.in/newsletters.html", (res) => {
    ...
    res.on("end", () => {
      ...
      if (h6Matches) {
        h6Matches.forEach((h6Text, index) => {
          ...
          paragraphs.forEach((p, pIndex) => {
            ...
            if (text.length > 10) {
              // Filter out short labels like "HPRC"
          ...
          const dateMatch = section.match(
            /(\d{1,2}\s+(?:January|February|March|April|May|June|July|August|September|October|November|December)[a-z]+\s+)(?:st|nd|rd|th)(?:\s+\d{1,2})/i,
          );
          if (dateMatch) {
            date = dateMatch[1];
          }
      ...
      const uniqueNewsletters = newsletters.filter((item, index, self) => {
        return (
          index === newsletters.findIndex((n) => n.title === item.title && n.date === item.date)
        );
      });
