...
https.get('https://hprc.in/newsletters.html', (res) => {
  ...
  res.on('end', () => {
    ...
    newsletterTitles.forEach((title) => {
      ...
      const sectionMatch = data.match(new RegExp(`(${titleRegex.source}[\\s\\S]*?)(?=<h6|$)`, 'i'));
      ...
      if (sectionMatch) {
        ...
        while ((imgMatch = imgPattern.exec(section)) !== null) {
          ...
          if (!src.includes('logo') && !src.includes('icon') && !src.includes('favicon') && 
              ...
              (src.includes('Newsletter') || src.includes('Equestrian') || src.includes('Horse') || 
               src.includes('Championship') || src.match(/\.(jpg|jpeg|png|gif)$/i))) {
            images.push(src);
          }
        }
        ...
        const fullPdfUrl = pdfUrl ? (pdfUrl.startsWith('http') ? pdfUrl : `https://hprc.in${pdfUrl.startsWith('/') ? '' : '/'}${pdfUrl}`) : '';
    ...
    while ((imgMatch = imgPattern.exec(data)) !== null) {
      ...
      if (!src.includes('logo') && !src.includes('icon') && !src.includes('favicon') && 
          !src.includes('banner') && !src.includes('telephone') &&
          (src.includes('Newsletter') || src.includes('Equestrian') || src.includes('Horse') || 
           src.includes('Championship') || src.includes('pdf') || src.match(/\.(jpg|jpeg|png|gif)$/i))) {
