...
https
  .get("https://hprc.in/newsletters.html", (res) => {
    ...
    res.on("end", () => {
      ...
      const linkPattern =
        /<a[^>]*href=['"]([^'"]*(?:Newsletter|newsletter|Equestrian|Horse|Championship)[^'"]*\.(?:html|pdf))['"][^>]*>([^<]*)<\/a>/gi;
      let linkMatch;
      ...
      while ((linkMatch = linkPattern.exec(data)) !== null) {
        ...
        if (
          ...
          (linkText.includes("NEWSLETTER") ||
            linkText.includes("EQUESTRIAN") ||
            linkText.includes("HORSE SHOW"))
        ) {
          ...
          if (!existing) {
            newsletters.push({
              ...
              url: href.startsWith("http")
                ? href
                : `https://hprc.in${href.startsWith("/") ? "" : "/"}${href}`,
            });
          } else if (href && !existing.url) {
            existing.url = href.startsWith("http")
              ? href
              : `https://hprc.in${href.startsWith("/") ? "" : "/"}${href}`;
          }
        }
