...
if ($name === '' || $mobile === '' || $email === '' || $selectedEvents === '') {
    error_log("TSEC2026: Rejected incomplete submission (missing required fields).");
...
if (isset($_FILES['ageProof']) && $_FILES['ageProof']['error'] == 0) {
    $uploadDir = __DIR__ . '/uploads/tsec2026/age_proofs/';
    if (!is_dir($uploadDir)) {
        if (!mkdir($uploadDir, 0755, true)) {
            error_log("TSEC2026: Failed to create upload directory: " . $uploadDir);
        }
    }
    ...
    if (move_uploaded_file($_FILES['ageProof']['tmp_name'], $targetPath)) {
        $ageProofPath = 'uploads/tsec2026/age_proofs/' . $fileName;
    } else {
        error_log("TSEC2026: Failed to move uploaded file. Error Code: " . $_FILES['ageProof']['error'] . " | Target: " . $targetPath);
    }
} else if (isset($_FILES['ageProof']) && $_FILES['ageProof']['error'] != 4) {
    error_log("TSEC2026: File upload error detected. Code: " . $_FILES['ageProof']['error']);
}

// Handle Rider Headshot Upload
$headshotPath = '';
if (isset($_FILES['headshot']) && $_FILES['headshot']['error'] == 0) {
    $headshotDir = __DIR__ . '/uploads/tsec2026/headshots/';
    if (!is_dir($headshotDir)) {
        if (!mkdir($headshotDir, 0755, true)) {
            error_log("TSEC2026: Failed to create headshot directory: " . $headshotDir);
        }
    }
    ...
    if (move_uploaded_file($_FILES['headshot']['tmp_name'], $targetPath)) {
        $headshotPath = 'uploads/tsec2026/headshots/' . $fileName;
    } else {
        error_log("TSEC2026: Failed to move headshot. Error Code: " . $_FILES['headshot']['error'] . " | Target: " . $targetPath);
    }
} else if (isset($_FILES['headshot']) && $_FILES['headshot']['error'] != 4) {
    error_log("TSEC2026: Headshot upload error detected. Code: " . $_FILES['headshot']['error']);
}
...
if ($stablingType !== 'NONE' && (int)$stablingCount > 0 && !empty($stablingFrom) && !empty($stablingTo)) {
    ...
    if (!$check['ok']) {
        ...
        error_log("TSEC2026: Stabling rejected for $name — $reason");
        $back = '/events/telangana-state-championship-2026?stabling_error=' . urlencode($reason);
        header("Location: $back");
        exit();
    }
}

$sql = "INSERT INTO tsec2026 (name, parentName, dob, address, mobile, email, emergencyContact, emergencyRelation, clubName, selectedEvents, eventHorses, horseBreeds, stablingType, stablingCount, stablingFrom, stablingTo, ageProofPath, headshotPath, amount, currency)
...
$merchant_data.='redirect_url=https://hprc.in/payment/tsec2026ResponseHandler.php&';
$merchant_data.='cancel_url=https://hprc.in/payment/tsec2026ResponseHandler.php&';
...
// Event labels for the Telangana State Championship (16 classes).
$eventMapping = [
...
if ($amount <= 0) {
    $conn->query("UPDATE tsec2026 SET order_status='Success', payment_mode='COMPLIMENTARY', trans_date='".date('Y-m-d H:i:s')."', status_message='Cheat Code Used' WHERE id='$order_id'");

    if ($stablingType !== 'NONE' && (int)$stablingCount > 0 && !empty($stablingFrom) && !empty($stablingTo)) {
        $imComp = new InventoryManagerOctCamp();
        $imComp->deductInventory($order_id, $name, (int)$stablingCount, $stablingFrom, $stablingTo, 'tsec');
    }
    ...
    foreach ($selectedIds as $id) {
        ...
        foreach ($horses as $jumpIdx => $horse) {
            ...
            $webhookPayloads[] = array(
                ...
                "amount" => "0 (COMP)", "tracking_id" => "HPRCTSECCHEAT-" . $id . "-J" . $jumpNumber,
    ...
    send_admin_notification("COMPLIMENTARY REGISTRATION (TSEC): $name (#$order_id)", $adminHtml, $attachment);
    ...
    header("Location: /events/telangana-state-championship-2026/success?status=Success&order_id=$order_id&amount=0&tracking_id=COMP-ENTRY");
    exit();
}
?>
<html>
